...
fn disk_name(disk: &sysinfo::Disk) -> String {
    ...
    if name.is_empty() || name.eq_ignore_ascii_case(&letter) { letter } else { format!("{letter} {name}") }
}

fn retain_selected_disk_ids(selected_ids: &[String], valid_ids: &[String]) -> Vec<String> {
    selected_ids.iter().filter(|id| valid_ids.iter().any(|valid_id| valid_id == *id)).cloned().collect()
}
...
pub fn refresh_disk_menu(view: &ui::AppWindow) {
    ...
    let disk_options = { disks.iter().map(|disk| (disk_id(disk), disk_name(disk))).collect::<Vec<_>>() };

    let valid_ids = disk_options.iter().map(|(id, _)| id.clone()).collect::<Vec<_>>();
    ...
    pool.spawn(move || {
        ...
        loop {
            ...
            if request.show_disk_total || request.show_disk_io || had_monitored_disks {
                ...
                let (next_disk_usage, next_disk_total_read, next_disk_total_write, next_monitored_disks, next_has_monitored_disks, next_selected_disk_ids) = {
                    ...
                    let disk_usage = if request.show_disk_total {
                        let (total_space, used_space) = disks.iter().fold((0u64, 0u64), |(total, used), disk| (total + disk.total_space(), used + disk.total_space().saturating_sub(disk.available_space())));
                        if total_space == 0 { 0.0 } else { (used_space as f32 / total_space as f32) * 100.0 }
                    } else {
                        0.0
                    };
                    ...
                    if request.show_disk_io || has_monitored_disks {
                        for disk in disks.iter() {
                            ...
                            if selected_ids.iter().any(|selected| selected == &id) {
                                let percent = if disk.total_space() == 0 { 0.0 } else { ((disk.total_space().saturating_sub(disk.available_space())) as f32 / disk.total_space() as f32) * 100.0 };
                    ...
                    (disk_usage, if request.show_disk_io { format_rate(total_read) } else { _zero_rate.to_string() }, if request.show_disk_io { format_rate(total_write) } else { _zero_rate.to_string() }, monitored, has_monitored_disks, selected_ids)
                };
            ...
            let (network_rx, network_tx) = if request.show_network {
                ...
                let (rx, tx) = networks.iter().fold((0u64, 0u64), |(received, sent), (_, data)| (received + data.received(), sent + data.transmitted()));
                ...
                if had_monitored_disks {
                    ...
                    for i in 0..model.row_count() {
                        if let Some(mut entry) = model.row_data(i) {
                            entry.checked = selected_disk_ids.iter().any(|selected_id| selected_id == &entry.id.to_string());
                            entries.push(entry);
                        }
                    }
                }
