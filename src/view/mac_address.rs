...
impl ViewTrait for MacAddressView {
    ...
    fn show(&self, _extra: Option<&dyn std::any::Any>) -> Result<()> {
        ...
        let _ = tokio::spawn(async move {
            let adapters = tools::network_adapters_async().await;
            let _ = slint::invoke_from_event_loop(move || {
                ...
                match adapters {
                    Ok(adapters) => {
                        ...
                        ui.set_original_mac(entries.first().map(|entry| entry.mac.clone()).unwrap_or_default());
                    }
                    Err(err) => {
    ...
    fn bind_event(self) -> Self {
        ...
        self.ui.on_apply_mac_address(|adapter_id, original_mac, new_mac| {
            let weak = ui::use_view::<crate::view::MacAddressView>().ui.as_weak();
            let _ = tokio::spawn(async move {
                let result = tools::set_mac_address_async(adapter_id.to_string(), original_mac.to_string(), new_mac.to_string()).await;
                let adapters = if result.is_ok() { tools::network_adapters_async().await } else { Ok(Vec::new()) };
                let _ = slint::invoke_from_event_loop(move || {
                    let Some(ui) = weak.upgrade() else { return; };
                    match result {
                        Ok(()) => {
                            MacAddressView::update_adapters(&ui, adapters);
                            ui.set_new_mac("".into());
                            ui.set_pending_adapter_id(adapter_id);
                            ui.set_success_dialog_visible(true);
                        }
                        Err(err) => {
        ...
        self.ui.on_restart_adapter(|adapter_id| {
            let weak = ui::use_view::<crate::view::MacAddressView>().ui.as_weak();
            let _ = tokio::spawn(async move {
                let result = tools::restart_network_adapter_async(adapter_id.to_string()).await;
                let adapters = if result.is_ok() { tools::network_adapters_async().await } else { Ok(Vec::new()) };
                let _ = slint::invoke_from_event_loop(move || {
                    let Some(ui) = weak.upgrade() else { return; };
                    match result {
                        Ok(()) => {
                            MacAddressView::update_adapters(&ui, adapters);
                            ui.set_status_text("网卡已重启".into());
                        }
                        Err(err) => {
                            error!("restart network adapter failed: {}", err);
                            ui.set_status_text(format!("重启失败: {err}").into());
                        }
                    }
                });
            });
...
impl MacAddressView {
    fn update_adapters(ui: &ui::MacAddressWindow, adapters: Result<Vec<tools::NetworkAdapter>>) {
        match adapters {
            Ok(adapters) => {
                ...
                ui.set_selected_index(0);
                ui.set_original_mac(entries.first().map(|entry| entry.mac.clone()).unwrap_or_default());
                ui.set_status_text(if entries.is_empty() { "没有找到可用网卡".into() } else { "".into() });
            }
            Err(err) => {
                error!("load network adapters failed: {}", err);
